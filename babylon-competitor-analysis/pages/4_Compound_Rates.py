"""Compound V3 — Stablecoin Borrow Rates (BTC Collateral) vs BTC Price.

Reads pre-collected data from data/compound_btc_borrow.csv and data/btc_price.csv.
...
st.set_page_config(page_title="Compound Rates", page_icon="🟢", layout="wide")
...
st.title("Compound V3 — Stablecoin Borrow Rates")
st.caption(
    "Variable borrow APY for USDC & USDT on Compound V3 (Ethereum) — "
    "borrowing stablecoins against BTC collateral (wBTC, cbBTC). "
    "Weekly on-chain data via The Graph (Paperclip Labs subgraph), last 2 years."
)
...
def load_compound() -> pd.DataFrame:
    path = DATA_DIR / "compound_btc_borrow.csv"
    if not path.exists():
        return pd.DataFrame()
...
if df.empty:
    st.error("No Compound rate data found. Run `uv run python collect_data.py` to collect data first.")
    st.stop()
...
with cols[min(len(symbols_present) + 1, len(cols) - 1)]:
    st.metric("Avg Borrow", f"{df['borrow_apy'].mean():.2f}%")
...
if not usdc_corr.empty and not btc_df.empty:
    ...
    if "btc_price" in merged.columns and len(merged) > 10:
        ...
        if abs(corr) > 0.3:
            ...
            st.info("Weak correlation — Compound rates driven more by pool utilization than BTC price directly.")
...
with st.expander("Raw weekly data"):
    tab_names = [s for s in symbols_present] + ["BTC Price"]
    tabs = st.tabs(tab_names)

    for i, sym in enumerate(symbols_present):
        with tabs[i]:
            group = df[df["symbol"] == sym][["date", "borrow_apy", "utilization"]].copy()
            group.columns = ["Date", "Borrow APY %", "Utilization %"]
...
st.caption(
    "**Data source:** Compound V3 (Comet) Ethereum on-chain rates from [The Graph](https://thegraph.com) "
    "(Paperclip Labs subgraph — weeklyMarketAccounting, USDC & USDT Comet markets). "
    "wBTC and cbBTC accepted as collateral. "
